...
# Objective Function
minimize Cost:
	0.7 * (sum{n in 1..number_of_drones, i in 1..grid_size, j in 1..grid_size, k in 1..grid_size, l in 1..grid_size, t in 1..time-1} distance_matrix[i, j, k, l] * nodes[n, i, j, t] * nodes[n, k, l, t + 1]) + 0.3 * ((max{t in 2..time} t * flag[1, t - 1] * (1 - flag[1, t])) + (max{t in 2..time} t * flag[2, t - 1] * (1 - flag[2, t])));
...
subject to Path_Connected{n in 1..number_of_drones, i in 2..grid_size-1, j in 2..grid_size-1, t in 1..time-1}:
	if nodes[n, i, j, t] = 1 
	then (sum{k in i-1..i+1, l in j-1..j+1} nodes[n, k, l, t+1]) - 1 = 0;
...
subject to Flag_Check_Delivery{n in 1..number_of_drones, t in 1..time-1}:
	if flag[n, t] = 0
	then 
		if (sum{k in 1..number_of_delivery_location} nodes[n, delivery_location[2 * k - 1], delivery_location[2 * k], t + 1] = 1)
		then (flag[n, t + 1] - 1)
		else flag[n, t + 1]
	=
	0;
...
subject to Flag_Check_Return{n in 1..number_of_drones, t in 1..time-1}:
	if flag[n, t] = 1
	then 
		if nodes[n, warehouse_location[2 * n - 1], warehouse_location[2 * n], t + 1]  = 1
		then flag[n, t + 1]
		else (1 - flag[n, t + 1])
	=
	0;
...
subject to Supply_Warehouse:
	sum{n in 1..number_of_drones, t in 2..time, k in 1..number_of_drones} nodes[n, warehouse_location[2 * k - 1], warehouse_location[2 * k], t] * (1 - flag[n, t]) * flag[n, t - 1] = net_demand;
	
# Constraint 9 - Demand Satisfaction Delivery Location
subject to Demand_Delivery_Location{n in 1..number_of_delivery_location}:
	sum{k in 1..number_of_drones, t in 2..time} nodes[k, delivery_location[2 * n - 1], delivery_location[2 * n], t] * flag[k, t] * (1 - flag[k, t - 1]) = delivery_demand[n];

# Constraint 10 - Collision Avoidance for drones 1 & 2
