...
def ProcessAMPLOutput(nodesfile):
    ...
    for i in range(time):
        for j in range(number_of_drones):
            nodes_temp[i][int(nodes_string[5 * i + j + 1].split()[0]) - 1][int(nodes_string[5 * i + j + 1].split()[1]) - 2][int(nodes_string[5 * i + j + 1].split()[2]) - 2] = 1
    ...
    for i in range(time):
        for j in range(number_of_drones):
            ...
            for k in range(grid_size):
                for l in range(grid_size):
                    if (k, l) == (2, 3) or (k, l) == (6, 6):
                        nodes[k][l] = 3
                    if (k, l) == (2, 0) or (k, l) == (4, 5) or (k, l) == (5, 7):
                        nodes[k][l] = 6
                    if nodes_temp[i][0][k][l] == 1:
                        nodes[k][l] = 2
                    else:
                        if nodes_temp[i][1][k][l] == 1:
                            nodes[k][l] = 4
...
# ProcessAMPLOutput('nodes_mm.txt')
